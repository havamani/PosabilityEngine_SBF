...
public class BenefitApi extends AbstractHostApi {
	...
	private void copyFieldsToBuffer(String prefix, IsoBuffer buffer,
	...
	protected String build(IsoBuffer isoBuffer) {
		...
		BenefitMessage message = new BenefitMessage();
		for (int i = 1; i <= 128; i++)
			message.setElement(FIELD_PREFIX + i, DISABLED);

		message.setElement(com.fss.pos.host.iso8583.benefit.Constants.MSG_TYP, isoBuffer.get(Constants.ISO_MSG_TYPE));
	...
	private void copyFieldsFromBuffer(String prefix, IsoBuffer isoBuffer,
			BenefitMessage message, int startPos, int endPos) {
		for (int i = startPos; i <= endPos; i++) {
			String key = prefix + i;
			if (CONTAINS_SUB_FIELDS_DE.contains(key)) {
