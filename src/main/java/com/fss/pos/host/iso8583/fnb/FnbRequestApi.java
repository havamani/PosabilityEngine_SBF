...
@HostRequest(HostType = "16")
public class FnbRequestApi extends Base24Api {
	...
	public void modify4All(IsoBuffer isoBuffer, Data hostClientData) {
		...
		isoBuffer.put(Constants.DE43, de43.toString());*/ //commented for iso testing
		...
		isoBuffer.put(
				Constants.DE48,
				Util.appendChar(hostData.getMerchantId(), ' ', 19, false)
						+ hostData.getMcc() + hostData.getState()
		...
		isoBuffer.put(Constants.DE60, config.getHostFiid() + "0000000000  ");
		isoBuffer.put(Constants.DE61, config.getHostFiid() + "0000000        ");
...
//
			...
			// added as per FNB requirement
			String buildTerminalSerialNo = tranLogData.getTerSerialNo() != null ? Util
					.appendChar(tranLogData.getTerSerialNo(), ' ', 12, false)
					: Util.appendChar("", ' ', 12, false);
			String qp = "! QP00012 " + buildTerminalSerialNo;
			...
			tokenData.append(Util.appendChar(Integer.toString(tokenLenth), '0',
					5, true));
			int finalLen = 12 + bufferLen;
			tokenData.append(Util.appendChar(Integer.toString(finalLen), '0',
					5, true));
			tokenData.append(ci);
			tokenData.append(qk);
			tokenData.append(c0);
			...
			String b4 = getB4Token(tvMap, isoBuffer, data);
			// added as per FNB requirement
			String buildTerminalSerialNo = hostData.getTerSerialNo() != null ? Util
					.appendChar(hostData.getTerSerialNo(), ' ', 12, false)
					: Util.appendChar("", ' ', 12, false);
			String qp = "! QP00012 " + buildTerminalSerialNo;
			...
			tokenData.append(b4);
			tokenData.append(qp);
			Log.debug("Token 63", tokenData.toString());
			return tokenData.toString();
		}
	...
	public void modifyPurchace(IsoBuffer buffer, Data data) {
		...
		if (Constants.ENTRY_MODE_MAG_STRIPE.equals(buffer.get(Constants.DE22)
			...
			buffer.put(
					Constants.DE22,
					(String.valueOf(arr[1].charAt(4)).equals("2")) ? DE22_FALL_BACK
							+ buffer.get(Constants.DE22).substring(2, 3)
							: DE22_MAGSTRIPE_ID
									+ buffer.get(Constants.DE22)
											.substring(2, 3));
			/*
			 * if (String.valueOf(arr[1].charAt(4)).equals("2")) buffer.put(
			 * Constants.DE22, DE22_FALL_BACK +
			 * buffer.get(Constants.DE22).substring(2, 3)); else
			 * buffer.put(Constants.DE22, DE22_MAGSTRIPE_ID +
			 * buffer.get(Constants.DE22).substring(2, 3));
			 */
	...
	public void modifyEmv(IsoBuffer isoBuffer, Data data) throws PosException,
			JSONException {
		...
		if (isoBuffer.get(Constants.ISO_MSG_TYPE).equals(
			...
			String b2 = getB2Token(tvMap, data,
			...
			emvData.append(Util.appendChar(Integer.toString(tokenLenth), '0',
					5, true));
			int finalLen = 12 + bufferLen;
			emvData.append(Util.appendChar(Integer.toString(finalLen), '0', 5,
					true));
			emvData.append(b2);
			emvData.append(b3);
			emvData.append(b4);
	...
	public void buildKeyImportRequest(IsoBuffer isoBuffer, Data data)
		...
		isoBuffer.put(Constants.ISO_MSG_TYPE,
				Constants.MSG_TYPE_RSP_NETWORK_MANAGEMENT);

		if (!(isoBuffer.get(Constants.DE39).equals(Constants.SUCCESS)))
			isoBuffer.put(Constants.DE123,
					"CSM(MCL/ESM RCV/FNB ORG/NMAD ERF/C )");
		else
			isoBuffer.put(Constants.DE123, "CSM(MCL/RSM RCV/FNB ORG/NMAD )");
	}

	@ModifyTransaction(Transactions.ADVICE)
	public void modifyAdvice(IsoBuffer buffer, Data data) throws PosException {
		...
		if (!Util.isNullOrEmpty(hd.getRespDe15())) {
			buffer.put(Constants.DE15, hd.getRespDe15());
		}

		buffer.put(Constants.DE27, "6"); // maximum length of the authorization
											// response that the acquirer can
											// accommodate.

		buffer.put(Constants.DE37, hd.getOrgRrn());
		buffer.put(Constants.DE38, hd.getOrgAuthCode());
		buffer.put(Constants.DE39, "00");
		// buffer.put(Constants.DE42, hd.getMerchantId());

		// buffer.put(Constants.DE54, "");

		buffer.put(Constants.DE63, get_token_63(buffer, data));
		if (!Util.isNullOrEmpty(hd.getRespDe100())) {
			buffer.put(Constants.DE100, hd.getRespDe100());
		}
		...
		buffer.disableField(Constants.DE47);
		buffer.disableField(Constants.DE62);
		buffer.disableField(Constants.DE123);
	...
	public void modifyReversal(IsoBuffer isoBuffer, Data data)
		...
		s90.append(tranLogData.getMerchantDate());
		isoBuffer.put(Constants.DE90,
				Util.appendChar(s90.toString(), '0', 42, false));
		if (Constants.ENTRY_MODE_MAG_STRIPE.equals(isoBuffer
				.get(Constants.DE22).substring(0, 2))) {
			String[] arr = isoBuffer.get(Constants.DE35).split("=");
			if (String.valueOf(arr[2].charAt(4)).equals("2"))
				isoBuffer
						.put(Constants.DE22,
								DE22_FALL_BACK
				...
				isoBuffer.put(Constants.DE22, DE22_MAGSTRIPE_ID
		...
		isoBuffer.put(Constants.DE37,
				Util.appendChar(tranLogData.getRrn(), '0', 12, true));

		isoBuffer.put(Constants.DE43, tranLogData.getReqDe43());

		String currencyCode = Util.appendChar(tranLogData.getCurrencyCode(),
				'0', 3, true);
		isoBuffer.put(Constants.DE49, currencyCode);
		...
		if (Constants.ENTRY_MODE_ICC_CB.equals(isoBuffer.get(Constants.DE22)
				.substring(0, 2)))
