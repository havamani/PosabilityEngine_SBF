package com.fss.pos.host.iso8583.standard;
...
@HostRequest(HostType = "15")
public class StandardBankRequestApi extends PostilionApi{
	...
	public void modify4All(IsoBuffer isoBuffer, Data data) {
		...
		de43.append((hostData.getCity().length() > 13) ?
				hostData.getCity()
		...
		if (!isoBuffer.isFieldEmpty(Constants.DE52))
			isoBuffer.put(Constants.DE52,
					SecurityService.hex2Alpha(isoBuffer.get(Constants.DE52)));
	...
	private void modifyEmv(IsoBuffer isoBuffer, Map<EmvTags, String> tvMap) {
		Log.debug("EMV Tags and values ", tvMap.toString());
		if (!tvMap.isEmpty()) {
			...
			//disable emv tag 9F03 for purchase with tip//
			if(isoBuffer.get(Constants.DE3).substring(0, 2).equals("00"))
				isoBuffer127_25.disableField(Constants.DE3);
	...
	public void modifyReversal(IsoBuffer isoBuffer, Data data) {
		...
		s90.append(isoBuffer.get(Constants.DE13));
		s90.append(isoBuffer.get(Constants.DE12));
		...
		if (null == trd.getRespDe54() || trd.getRespDe54().equals("")) {

		} else {
			isoBuffer.put(Constants.DE54, trd.getRespDe54());
			Log.debug("cash back reversal",
					"de54 data filled" + isoBuffer.get(Constants.DE54));
		}
